...
class ModelArguments:
    ...
    align_mode: str = field(
        metadata={"choices" : WordLevelBert.MODES,
                  "help": "alignment mode"}
    )
    max_sent: int = field()
    sent_path: str = field()
    align_path: str = field()
    ...
    base_diff_wght: float = field(
        default=0.0,
        metadata={"help": "base-model divergence loss weight"}
    )
    align_wght: float = field(
        default=1.0,
        metadata={"help": "alignment loss weight"}
    )
    align_batch_size: int = field(
        default=8
    )
...
def main():
    ...
    align_sampler = RandomAlignedSentSampler(sent_path=model_args.sent_path,
                                           align_path=model_args.align_path,
                                           max_sent_qty=model_args.max_sent,
                                           lang=model_args.align_language)

    model_align = WordLevelBert(model_args.align_mode,
                                MBERT_MODEL_NAME, MBERT_MODEL_NAME,
                                batch_size=model_args.align_batch_size)
    model_align.bert = model
    model_align.cuda()
    align_wght = model_args.align_wght
    base_diff_wght = model_args.base_diff_wght

    model_align_base = None
    if base_diff_wght > MIN_WGHT:
        model_align_base = WordLevelBert(model_args.align_mode,
                                MBERT_MODEL_NAME, MBERT_MODEL_NAME,
                                batch_size=model_args.align_batch_size)
    ...
    if training_args.do_train:
        ...
        logger.info(f"  Alignment weight: {align_wght} Base diff weight: {base_diff_wght}")
        ...
        for epoch in range(int(training_args.num_train_epochs)):
            for step, batch in enumerate(train_dataloader):
                ...
                align_batch = align_sampler.sample_batch(model_args.align_batch_size, include_clssep=model_args.include_clssep)
